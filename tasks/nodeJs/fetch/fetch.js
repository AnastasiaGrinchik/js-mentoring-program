...
const sendRequest = async () => {
    ...
    let newJson = await json.filter(function (item) {
        return item.id < 20;
    });

    let jsonStringified = await JSON.stringify(newJson, null, '\t');

    await fs.writeFile('./tasks/nodeJs/fetch/response.json', jsonStringified);
};
