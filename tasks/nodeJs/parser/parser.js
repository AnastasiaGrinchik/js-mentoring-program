...
const jsonParser = async () => {
    let testFile = await fs.readFile('./tasks/nodeJs/parser/test.json', 'utf8');
    const obj = await JSON.parse(testFile);
    let entries = await obj.list.entries;
    let exitArr = await entries.reduce(function (acc, el) {
        let currentName = el.entry.name + '';
        currentName = currentName.split('.html')[0];
        currentName = 'http://doc.epam.com/' + currentName;
        acc.push({ docId: currentName });
        return acc;
    }, []);

    let newJson = await JSON.stringify(exitArr, null, '\t');
    await fs.writeFile('./tasks/nodeJs/parser/parsed.json', newJson);
};
